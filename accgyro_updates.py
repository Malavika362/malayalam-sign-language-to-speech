...
def find_gesture(csv_file, input_values):
    ...
    try:
        ...
        input_list = input_list[:11]  # Use Flex1 to Flex5, AccX, AccY, AccZ, GyroX, GyroY, GyroZ
    except ValueError as e:
        return f"Error converting input to float: {e}"

    df = pd.read_csv(csv_file)
    for col in df.columns[:11]:  # Converting Flex1 to Flex5, AccX, AccY, AccZ, GyroX, GyroY, GyroZ
        df[col] = pd.to_numeric(df[col], errors='coerce')

    # Set tolerance values for each sensor
    tolerances = [0.20, 0.20, 0.20, 0.20, 0.20,  # Flex1 to Flex5
                  0.95, 0.95, 0.95,  # AccX, AccY, AccZ
                  0.95, 0.95, 0.95]  # GyroX, GyroY, GyroZ
